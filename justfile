...
check: fmt-check lint test site-test site-build
...
# Install pinned Timbermill dependencies
site-setup:
    cd site && npm_config_cache="${TMPDIR:-/tmp}/timbers-npm-cache" npm ci

# Test Timbermill content materialization
site-test:
    cd site && npm test

# Build the static Timbermill site
site-build:
    cd site && npm run build

# Preview the Timbermill site locally
site-serve:
...
release version:
    ...
    # Commit, tag, push
    git add CHANGELOG.md site/content/examples/
...
# Backward-compatible alias for the site preview
blog-serve: site-serve
...
clean:
    rm -rf bin/ dist/
    rm -rf site/.generated/ site/_site/
