...
|---|---|---|
| `docs/llm-commands.md:110-137` | Custom templates use Go-template expressions such as `{{.RepoName}}`, `{{range .Entries}}`, and field access. | Replace with supported literal tokens from `internal/draft/render.go`: `{{repo_name}}`, `{{branch}}`, `{{entry_count}}`, `{{entries_json}}`, `{{entries_summary}}`, `{{project_description}}`, `{{is_first_batch}}`, `{{total_entries}}`, `{{append}}`, and declared `{{vars.key}}`. State that iteration and conditionals belong in the LLM prompt, not a template engine. |
