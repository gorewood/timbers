...
The general lesson — the one I want to remember in six months — is that **when multiple code paths answer the same user-visible question, they need to share the function, not just the intent**. Comments saying "keep this in sync with X" are wishes. A single helper is a constraint. The bug existed because three call sites were each implementing "is this worth bothering the user about?" by hand, and one of them quietly forgot a clause.
